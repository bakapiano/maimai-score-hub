...
本文档根据当前代码实现整理 DXNet bot status 的上报、存储和使用方式。这里的 bot status 指 `bot_statuses` 集合中的 Bot 可用性状态，以及 Worker 随 status report 一起上报、后端写入 `bot_friend_snapshots` 的好友列表快照。

## 关键结论

- **Worker 是 bot status 的唯一主动上报方**：每个 DXNet worker 维护一个 bot cookie，并每 60 秒向后端上报一次。
- **`available` 表示 DXNet cookie / session 是否可用，不等于“某用户已是好友”**：用户与 Bot 是否为好友由 `bot_friend_snapshots` 里的好友列表快照判断。
- **后端读 status 时会做超时降级**：`BotStatusService.getAll()` 会把 5 分钟未上报的 bot 视为 `available=false`。
- **好友快照是 status report 的 side channel**：Worker 传 `friends` 时，后端 full-overwrite 写入 `bot_friend_snapshots`；没有 `friends` 时只更新 `bot_statuses`，不覆盖旧快照。
...
`friends` 不存入 `bot_statuses`。当请求里包含 `friends` 数组时，后端额外 full-overwrite `bot_friend_snapshots`：

| 字段 | 含义 |
| --- | --- |
| `botFriendCode` | Bot 好友码 |
| `friends[]` | 该 Bot 好友列表，只保存 `friendCode`、`userName`、`rating` |
| `updatedAt` | `friendsUpdatedAt` 可解析时使用它，否则使用当前时间 |

`bot_friend_snapshots.updatedAt` 有 30 天 TTL，避免长时间不再上报的 Bot 继续参与 QR-login 反查。
...
2. 对每个包含 `friends` 的 bot，调用 `BotFriendSnapshotService.report()` full-overwrite 好友快照。
...
| `JobService.getFriendshipStatus()` | 读取可用 Bot，按 `friendCount` 升序生成候选列表，再用 `bot_friend_snapshots` 判断当前用户是否已在某个可用 Bot 好友列表里。 |
...
| `Worker get_full_friend_list` job | 主动抓完整好友列表，通过 `getFriendList()` full-overwrite BotManager 内存快照；快照变化后由统一监听器触发 bot status report，刷新 `bot_statuses` 和 `bot_friend_snapshots`。 |
...
4. 在 `bot_friend_snapshots` 中查找任一可用 Bot 的 `friends.friendCode == 当前用户 friendCode`。
5. 如果找到：
   - `isFriend=true`
...
- `bot_friend_snapshots` 只有在 Worker 成功拿到好友列表并随 status report 发送 `friends` 时才更新；普通 status report 可以只更新可用性而不刷新 snapshot。
