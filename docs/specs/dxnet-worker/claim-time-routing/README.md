...
per-bot queue”迁移到“job 先进入 shared queue、BullMQ 选中 active worker 后绑定 Bot、再执行
cabinet prerequisite”的目标设计。

本目录定义 routing-v2 目标态与停机切换方案。以下旧文档仍可用于对照切换前行为：
...
2. **哪个 DXNet worker 被 BullMQ 选为 active processor，哪个 Bot 成为该次执行 Bot。**
   BullMQ lock 是唯一执行权；backend 不再实现第二套抢占或 lease，只通过现有 worker PATCH
   原子记录本次执行 Bot，并用 BullMQ delivery generation 拒绝旧执行写入。
3. **worker 在执行 handler 前触发 prerequisite。** worker 调 backend 的 prepare
   endpoint；backend 根据当前 execution Bot 和目标用户解析 cabinet id，enqueue sdgb
   `add_rival` 并等待结果。
...
8. **不新增 friendship cleanup 子系统。** claim 临时关系统一交给现有每 5 分钟
   CleanupService；它会先查询 active friend codes，避免上一 Job 的延迟 remove 删除下一 Job
   正在使用的关系。
9. **不新增 sdgb lane。** 所有 addRival 留在现有 Interactive queue，继承 parent priority，
   继续受 concurrency=1 和 Stable 0.5 QPS 限制。

## 文档

- [01-architecture.md](./01-architecture.md)：目标职责、BullMQ activation、PATCH/prepare
  流程和各业务路径。
- [02-queues-priority-concurrency.md](./02-queues-priority-concurrency.md)：BullMQ lane、
  priority 和 queue-level concurrency。
- [03-state-failure-idempotency.md](./03-state-failure-idempotency.md)：Mongo 状态、BullMQ
  delivery fencing、at-least-once、副作用与清理。
- [04-migration-testing-rollout.md](./04-migration-testing-rollout.md)：停机迁移、测试、指标和
  上线顺序。
...
- 不承诺 exactly-once。目标是在 at-least-once 下让 activation、prepare 和 cleanup 可恢复、
  可观测、尽量幂等。
