...
| `JobApiLogEntity` | `job_api_logs` | job/api-log | 24 小时 TTL | worker 处理 job 时的 API 调用日志 |
| `JobTempCacheEntity` | `job_temp_cache` | job/cache | 1 小时 TTL | `update_score` 中间结果缓存 |
...
| `SdgbWorkerStatusEntity` | `sdgb_worker_status` | sdgb-worker | 永久 | sdgb-worker 心跳 |
| `WorkerLogEntity` | `worker_logs` | worker-logs | 默认 2 小时 TTL | worker 控制台日志 |
...
| 字段 | 类型 | 约束 / 默认值 | 说明 |
| --- | --- | --- | --- |
| `jobId` | `string` | required, index | job id |
| `url` | `string` | required | 请求 URL |
| `method` | `string` | required | HTTP method |
| `statusCode` | `number` | required | 响应状态码 |
| `responseBody` | `string \| null` | default `null` | 响应 body |
| `createdAt` | `Date` | required, index | 创建时间 |

索引：

- `jobId`：单字段索引。
- `createdAt`：单字段索引 + TTL 24 小时。
...
| --- | --- | --- | --- |
| `jobId` | `string` | required, index | job id |
| `diff` | `number` | required | 难度 |
| `type` | `number` | required | `1 = dxScore`，`2 = score` |
| `songs` | `FriendVsSong[]` | required | friend-VS 中间结果 |
| `createdAt` | `Date` | required, index | 创建时间 |

索引：

- `createdAt`：单字段索引 + TTL 1 小时。
- `{ jobId: 1, diff: 1, type: 1 }`：唯一复合索引。
...
| 字段 | 类型 | 约束 / 默认值 | 说明 |
| --- | --- | --- | --- |
| `workerId` | `string` | required, unique, index | worker 标识 |
| `lastSeenAt` | `Date` | required | 最近心跳时间 |
| `jobsClaimed` | `number` | default `0` | 进程启动以来 claim 的 job 数 |
| `createdAt` | `Date` | timestamps | 创建时间 |
| `updatedAt` | `Date` | timestamps | 更新时间 |
...
来源：`backend/src/modules/worker-logs/worker-log.schema.ts`

### `WorkerLogEntity`

| 字段 | 类型 | 约束 / 默认值 | 说明 |
| --- | --- | --- | --- |
| `workerId` | `string` | required, index | worker 标识 |
| `workerKind` | `'sdgb' \| 'dxnet'` | required, index | worker 类型 |
| `ts` | `Date` | required, index | worker 侧日志时间 |
| `level` | `'log' \| 'warn' \| 'error'` | required | 日志等级 |
| `message` | `string` | required | 已格式化的日志内容 |

索引：

- `ts`：TTL，默认 2 小时，可由 `WORKER_LOGS_TTL_SECONDS` 调整。
- `{ workerId: 1, ts: -1 }`，名称 `by_worker_recent`。
- `{ workerKind: 1, ts: -1 }`，名称 `by_kind_recent`。
...
| `JobApiLogDocument` | `JobApiLogEntity` |
| `JobTempCacheDocument` | `JobTempCacheEntity` |
...
| `SdgbWorkerStatusDocument` | `SdgbWorkerStatusEntity` |
| `WorkerLogDocument` | `WorkerLogEntity` |
